...
async function getMenusWithItems() {
  const menus = await db.all('SELECT * FROM navigation_menus ORDER BY is_default DESC, name ASC')
...
router.post('/', authenticateToken, async (req, res) => {
  try {
    ...
    const insertResult = await db.run(
      'INSERT INTO navigation_menus (name, slug, is_default) VALUES (?, ?, ?)',
...
router.put('/:id', authenticateToken, async (req, res) => {
  try {
    ...
    if (name) {
      await db.run('UPDATE navigation_menus SET name = ?, is_default = COALESCE(?, is_default) WHERE id = ?', [
        name.trim(),
        isDefault ? 1 : null,
        id
      ])
    } else if (typeof isDefault !== 'undefined') {
      await db.run('UPDATE navigation_menus SET is_default = ? WHERE id = ?', [isDefault ? 1 : 0, id])
    }
...
router.delete('/:id', authenticateToken, async (req, res) => {
  try {
    const { id } = req.params
    const menu = await db.get('SELECT * FROM navigation_menus WHERE id = ?', [id])
    if (!menu) return res.status(404).json({ error: 'Menu not found' })
    if (menu.is_default) return res.status(400).json({ error: 'Default menu cannot be deleted' })

    await db.run('DELETE FROM navigation_items WHERE menu_id = ?', [id])
    await db.run('DELETE FROM navigation_menus WHERE id = ?', [id])
