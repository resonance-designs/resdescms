...
- Theme management: color controls aligned on a single row; added background image picker (upload or library) with repeat/size/position/attachment options; preset position dropdown.

## v0.2.6 (alpha)
- Admin polish: sidebar branding shows the app icon; logout/navigation/design/media/category controls now use Tabler icons with pointer cursors; design tabs and theme sub-tabs are iconized; media upload/delete and category add/delete buttons are iconized.
- Categories are now global (no type filtering); any category can be used for pages or posts.
...
- Admin IA refresh: content links grouped under a "Content" accordion; settings/users/plugins grouped under "Site"; navigation management moved into Design → Menus; home page picker now lists available pages.
