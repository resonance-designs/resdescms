...
```text
┌─────────────────────┐        HTTP/JSON        ┌─────────────────────┐│  Vue 3 SPA (Vite)   │  ───────────────────▶   │  Express API        ││  - Router/Pinia     │   fetch/axios           │  - Routes: auth,    ││  - Page/PageBuilder │                         │    posts, pages,    ││  - Admin views      │  ◀───────────────────   │    media, nav,      │└─────────┬───────────┘    responses            │    design, themes   │          │                                     └─────────┬───────────┘          │ static assets /themes, /uploads                │          ▼                                                ▼┌─────────────────────┐                        ┌─────────────────────┐│  Browser            │                        │  SQLite (cms.db)    ││  - Renders SPA      │                        │  - Content tables   ││  - Loads theme CSS  │                        │  - Navigation menus ││  - Loads media      │                        │  - Design/theme cfg │└─────────────────────┘                        └─────────────────────┘
```
